...
const DeleteDialog = ({ open, setOpen, imdbId, title }: Props) => {
  ...
  const removeEntry = async () => {
    try {
      const response = await sendRequest(`/api/list/entry/delete/${imdbId}`);
      setOpen(false);
                ...
                <div className="sm:flex w-full">
                  <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left sm:w-full">
                    ...
                    <div className="mt-2 sm:mt-6 sm:mb-2 p-1 flex w-full ">
                      ...
                      {!error && !isLoading && (
                        <>
                          <div className="flex basis-3/6 items-center justify-center p-1"></div>
                ...
                {!error && (
                  <>
                    <button
                      ...
                      className={`w-full sm:w-auto sm:text-sm font-medium inline-flex justify-center rounded-md text-base px-4 py-2 sm:ml-2 border  hover:bg-red-600 hover:border-transparent text-white bg-red-700 `}
                    >
                      Remove
                    </button>
                    <button
